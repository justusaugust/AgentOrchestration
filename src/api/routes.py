...
async def get_agent(agent_id: str):
    try:
        ...
        raise HTTPException(status_code=400, detail="Malformed agent id")
    if not agent:
...
async def start_agent(agent_id: str):
    try:
        updated = registry.update_status(agent_id, AgentStatus.RUNNING)
    except AgentIdValidationError:
        raise HTTPException(status_code=400, detail="Malformed agent id")
    if not updated:
        raise HTTPException(status_code=404, detail="Agent not found")
...
async def stop_agent(agent_id: str):
    try:
        updated = registry.update_status(agent_id, AgentStatus.PAUSED)
    except AgentIdValidationError:
        raise HTTPException(status_code=400, detail="Malformed agent id")
    if not updated:
        raise HTTPException(status_code=404, detail="Agent not found")
